...
- **Secure Media Storage** - Private Supabase Storage bucket for admin avatars with signed URL delivery, automatic refresh logic, and 4MB blog cover image support. Blog cover previews stay local until you save, so Supabase only stores the final chosen image.
