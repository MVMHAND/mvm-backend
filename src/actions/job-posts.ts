...
> {
  try {
    ...
    let query = supabase
      .from('job_posts')
      .select('*, category:job_categories(id, name, slug)', { count: 'exact' })
...
export async function getJobPostByIdAction(id: string): Promise<ActionResponse<JobPost>> {
  try {
    ...
    const { data, error } = await supabase
      .from('job_posts')
      .select('*, category:job_categories(id, name, slug)')
      .eq('id', id)
      .single()
...
export async function createJobPostAction(
  ...
  try {
    ...
    const responsibilities = formData.responsibilities
      ...
      : []
    const requirements = formData.requirements ? splitByNewline(formData.requirements) : []
...
export async function updateJobPostAction(
  ...
  try {
    ...
    const updateData: Record<string, unknown> = {
      title: formData.title,
      overview: formData.overview || null,
      cover_image_url: formData.cover_image_url || existingPost.cover_image_url,
